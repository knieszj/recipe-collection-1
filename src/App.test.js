...
test('toggleAddRecipeForm() modifies isAddRecipeFormDisplayed state value to toggle visibility of a form on the page', () => {
    ...
    expect(wrapper.state().isAddRecipeFormDisplay).toBeFalsy();
});
...
test('the Add Recipe button onClick calls the toggleAddRecipeForm method', () => {
    ...
    // the above comment is copy/pasted.
...
test('typing into the recipe instructions input updates state', () => {
    const wrapper = shallow(<App/>);
    const recipeInstructions = "kinda hard to write instructions without knowing what i'm cooking.  But it doesn't matter because I'm just going to burn it and order pizza anyway.";
